...
from . import utils, plot_utils, fp_utils
import numpy as np
import pandas as pd
...
def process_fp_data(fp_data, sess_ids, lig_lpf=10, iso_lpf=1):
    ''' Loads and processes fiber photometry data'''

    ligs = np.aray(['490', '465'])

    # process the signals
    for subj_id in sess_ids.keys():

        subj_isos = np.array(preferred_isos[subj_id])
