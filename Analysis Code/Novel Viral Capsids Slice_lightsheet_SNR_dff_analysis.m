...
%%% Nikki Tjahjono
%%% 07/22/21 Updated
classdef timeconffull_exported_chris < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function RunPixelWiseAnalysisButtonPushed(app, event)
            ...
            if app.GenerateHeatmapandHistogramCheckBox.Value
                ...
                hold off
                sigma = 3;
                smoothed_snr = imgaussfilt(snrs(:,:,app.StackEditField_2.Value), sigma);
                subplot(2, 2, 2) %changed from subplot(2,1,1)
                title('SNR heatmap')
                hold on
                colormap(jet(10));
                imagesc(smoothed_snr)
%                 imagesc(smoothed_snrs(:,:,app.StackEditField_2.Value))
                colorbar
                ...
                figure('Name', 'Smoothed dF/F Map', 'Units', 'inches', 'Position', [1 1 10 10]);
                hold on
                smoothed_dff = imgaussfilt(dffs(:,:,app.StackEditField_2.Value), sigma);
                imagesc(smoothed_dff);

                colormap hot;
                colorbar;
                caxis([0 app.MaxDFFEditField.Value]);
                title(sprintf('Smoothed dF/F (Frame %d)', app.StackEditField_2.Value), 'FontSize', 14);
                axis off;
                daspect([1 1 1]);
                hold off

                figure('Name', 'Smoothed SNR Map', 'Units', 'inches', 'Position', [1 1 10 10]);
                hold on
                smoothed_snr = imgaussfilt(snrs(:,:,app.StackEditField_2.Value), sigma);
                imagesc(smoothed_snr);
                ...
                colorbar;
                caxis([0 app.MaxSNREditField.Value]);
                title(sprintf('Smoothed SNR (Frame %d)', app.StackEditField_2.Value), 'FontSize', 14);
                axis off;
                daspect([1 1 1]);
                hold off
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = timeconffull_exported_chris
